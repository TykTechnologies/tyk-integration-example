...
generate_credentials(){
  ...
  export TYK_API_KEY=$USER_AUTH_CODE
...
create_organisation() {
  ORGDATA=$(curl --silent --header "admin-auth: 12345" --header "Content-Type:application/json" --data '{"owner_name": "Default Org.","owner_slug": "default", "cname_enabled": true, "cname": ""}' http://$1:3000/admin/organisations 2>&1)
...
user_data(){
  user_curl=$(curl --silent --header "admin-auth: 12345" --header "Content-Type:application/json" --data '{"first_name": "Joan","last_name": "Smith","email_address": "'$2'@default.com","password":"'$3'", "active": true,"org_id": "'$4'"}' http://$1:3000/admin/users 2>&1)
...
get_user_list(){
  list=$(curl --silent --header "authorization: $2" http://$1:3000/api/users 2>&1)
...
setting_password(){
  USER=$(get_user $1 $2)
  USERID=$(get_user_id "$USER")
  response=$(curl --silent --header "authorization: $2" --header "Content-Type:application/json" http://$1:3000/api/users/$USERID/actions/reset --data '{"new_password":"'$3'"}')
